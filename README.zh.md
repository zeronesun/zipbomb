...
| `zbxl.zip` | 46 MB | 4.5 PB | 98,000,000× | 超过 42.zip 解压后大小 | ✅ 已预生成（`zip/`） |
...
`zip/` 目录下已预生成 `zbsm.zip`、`zblg.zip` 与 `zbxl.zip` 三个示例炸弹（可直接用 `ratio` 分析，但**切勿解压**）。其余炸弹体积庞大，请用 `make` 按需生成，生成的是压缩归档文件本身、不会占用解压后的空间。
